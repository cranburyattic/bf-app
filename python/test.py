...
import yaml
import os
path = '/Users/phirow/Development/betfair-data/data/2016-7-25';
csv_files = [f for f in os.listdir(path) if f.endswith('.csv')]

#print(csv_files);

for filename in csv_files:
    race = pd.read_csv('/Users/phirow/Development/betfair-data/data/2016-7-25/' + filename, header=None)
    print(filename)
    race.columns = ['odds','matched','back','lay']
    odds = race[['odds']]
    odds_a = race.plot(kind='line')
    odds_a.get_figure().savefig('/tmp/' + filename + '.png')
    print(race.head(400).describe().loc[['min','max']])
    print('')
